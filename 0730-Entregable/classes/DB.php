	...
	abstract class DB
	{
		...
		public static function getAllActors()
		{
			...
			$stmt = $connection->prepare("
					...
					FROM actors A LEFT JOIN movies M ON A.favorite_movie_id = M.id");
			$stmt->execute();
			...
			foreach ($actors as $actor) {
				$cActors = new Actor($actor['first_name'], $actor['last_name'], $actor['rating']);
				$cActors->setId($actor['id']);
				$cActors->setFavoriteMovieId($actor['title']);
				$objActors[] = $cActors;
			}
		...
		public static function saveActor(Actor $actor): bool
		{
			...
			try {
				$stmt = $connection->prepare("INSERT INTO actors (first_name, last_name, rating, favorite_movie_id) 
